...
module.exports = function(s3Config, logConfig, log) {
  ...
  wk.processFile = function(filePath, runConfig, cb) {
    if (filePath && filePath.length > 1) {
      ...
      let daily = dailyLogPath + day+'_daily' + (runConfig.dailyLogPrefix ? '_' + runConfig.dailyLogPrefix : '' ) + '.log';
      ...
      async.series({
        ...
        json.on('end', function() {
          if (!runConfig.moveProcessed) return cb()
  ...
  wk.run = function(runConfig, callback) {
    ...
    wk.listFiles(listParams, function(err, paths) {
      ...
      async.eachSeries(paths, function(path, itDone) {
        wk.processFile(_.get(path, 'Key'), runConfig, function(err, result) {
